...
uses Classes, SysUtils, Math, FEMModel, FEMTypes, FEMAnalysisCases;
...
begin
  ...
  for I:=0 to High(M.Elements) do begin
    ...
    if M.Elements[I].ID<=0 then Error('BAD_ELEMENT_ID',Format('Element has non-positive ID %d.',[M.Elements[I].ID]));
    if Trim(M.Elements[I].Kind)='' then Error('ELEMENT_KIND_EMPTY',Format('Element %d has no element type.',[M.Elements[I].ID]));
    if Length(M.Elements[I].NodeIDs)=0 then Error('ELEMENT_NO_NODES',Format('Element %d has no nodes.',[M.Elements[I].ID]));
    if SameText(M.Elements[I].Kind,'BEAM3D') and (Length(M.Elements[I].NodeIDs)<>2) then
      ErrorEntity('BEAM3D_NODE_COUNT',Format('BEAM3D element %d requires exactly 2 nodes; found %d.',[M.Elements[I].ID,Length(M.Elements[I].NodeIDs)]),'Element',M.Elements[I].ID)
    else if SameText(M.Elements[I].Kind,'TRI3_MEMBRANE') and (Length(M.Elements[I].NodeIDs)<>3) then
      ErrorEntity('TRI3_NODE_COUNT',Format('TRI3_MEMBRANE element %d requires exactly 3 nodes; found %d.',[M.Elements[I].ID,Length(M.Elements[I].NodeIDs)]),'Element',M.Elements[I].ID)
    else if not SameText(M.Elements[I].Kind,'BEAM3D') and not SameText(M.Elements[I].Kind,'TRI3_MEMBRANE') then
      ErrorEntity('UNKNOWN_ELEMENT_KIND',Format('Element %d uses unsupported element type "%s".',[M.Elements[I].ID,M.Elements[I].Kind]),'Element',M.Elements[I].ID);
  ...
  for I:=0 to High(M.Loads) do begin
    for J:=0 to I-1 do if M.Loads[J].ID=M.Loads[I].ID then Error('DUP_LOAD_ID',Format('Duplicate load ID %d.',[M.Loads[I].ID]));
    if M.Loads[I].ID<=0 then Error('BAD_LOAD_ID',Format('Load has non-positive ID %d.',[M.Loads[I].ID]));
    ...
    for J:=0 to 5 do if IsNan(M.Loads[I].Value[J]) or IsInfinite(M.Loads[I].Value[J]) then Error('BAD_LOAD_VALUE',Format('Load %d has a non-finite DOF value.',[M.Loads[I].ID]));
  end;

  for I:=0 to M.AnalysisCases.Count-1 do begin
    if M.AnalysisCases.Item(I).ID<=0 then Error('BAD_ANALYSIS_ID',Format('Analysis case has non-positive ID %d.',[M.AnalysisCases.Item(I).ID]));
    for J:=0 to I-1 do if M.AnalysisCases.Item(J).ID=M.AnalysisCases.Item(I).ID then Error('DUP_ANALYSIS_ID',Format('Duplicate analysis case ID %d.',[M.AnalysisCases.Item(I).ID]));
    if M.FindLoadCase(M.AnalysisCases.Item(I).LoadCaseID)<0 then ErrorEntity('ANALYSIS_LOADCASE_MISSING',Format('Analysis case %d references missing load case %d.',[M.AnalysisCases.Item(I).ID,M.AnalysisCases.Item(I).LoadCaseID]),'Analysis',M.AnalysisCases.Item(I).ID);
    if M.AnalysisCases.Item(I).Settings=nil then ErrorEntity('ANALYSIS_SETTINGS_MISSING',Format('Analysis case %d has no settings object.',[M.AnalysisCases.Item(I).ID]),'Analysis',M.AnalysisCases.Item(I).ID)
    else if M.AnalysisCases.Item(I).AnalysisType=atLinearStatic then begin
      if Trim(TLinearStaticSettings(M.AnalysisCases.Item(I).Settings).SolverName)='' then ErrorEntity('ANALYSIS_SOLVER_EMPTY','Linear-static analysis has no solver name.','Analysis',M.AnalysisCases.Item(I).ID);
      if TLinearStaticSettings(M.AnalysisCases.Item(I).Settings).PivotTolerance<=0 then ErrorEntity('ANALYSIS_PIVOT_TOLERANCE','Linear-static pivot tolerance must be positive.','Analysis',M.AnalysisCases.Item(I).ID);
      if TLinearStaticSettings(M.AnalysisCases.Item(I).Settings).ResidualTolerance<=0 then ErrorEntity('ANALYSIS_RESIDUAL_TOLERANCE','Linear-static residual tolerance must be positive.','Analysis',M.AnalysisCases.Item(I).ID);
    end;
  end;
  Result:=ErrorCount=0;
end;
