...
begin
  ...
  if not SameText(LastResult.Status,'OK') then begin
    ResultInfo.Caption:='Result status: '+LastResult.Status;
    Inspector.Lines.Add(''); Inspector.Lines.Add('SOLVER DIAGNOSTICS'); Inspector.Lines.Add('--------------------------------');
    if LastResult.Errors.Count=0 then Inspector.Lines.Add('  '+LastResult.Status)
    else Inspector.Lines.AddStrings(LastResult.Errors);
    Inspector.Lines.AddStrings(LastResult.AuditTrail);
    Exit;
  end;
...
begin
  ...
  if CurrentModelFile='' then begin
    SaveDlg.Filter:='FEM3D model|*.fem3d|All files|*.*';
    if not SaveDlg.Execute then Exit;
    CurrentModelFile:=SaveDlg.FileName;
  end;
  InputFile:=CurrentModelFile; OutFile:=ChangeFileExt(InputFile,'.fem3dres');
  ...
  try
    TFEMNativeIO.SaveModel(Model,InputFile);
    SetLength(Params,1); Params[0]:=InputFile;
    if not TSolverProcessRunner.Run(SolverFile,Params,PR) then begin Log.Lines.Add(PR.ErrorText); Exit; end;
    if PR.OutputText<>'' then Log.Lines.Add(Trim(PR.OutputText));
    if PR.ErrorText<>'' then Log.Lines.Add('Solver stderr: '+Trim(PR.ErrorText));
    if FileExists(OutFile) then LoadResultFile(OutFile);
    A.ResultID:=ExtractFileName(OutFile);
    if PR.ExitCode<>0 then Log.Lines.Add(Format('Solver process failed with exit code %d; diagnostic result loaded from %s',[PR.ExitCode,OutFile]))
    else Log.Lines.Add('Linear Static solver completed: '+OutFile);
...
begin
  SaveDlg.Filter:='FEM3D model|*.fem3d|All files|*.*';
  if SaveDlg.Execute then begin try TFEMNativeIO.SaveModel(Model,SaveDlg.FileName); CurrentModelFile:=SaveDlg.FileName; Log.Lines.Add('Saved '+SaveDlg.FileName);except on E:Exception do Log.Lines.Add('Save failed: '+E.Message);end;end;
end;
