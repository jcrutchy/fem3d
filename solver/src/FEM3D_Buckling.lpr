program FEM3D_Buckling;
{$mode objfpc}
uses Classes, SysUtils, FEMModel, FEMElements, FEMAnalysisCases, FEMValidation, FEMIO, FEMHash;

procedure Info;
begin
  Writeln('FEM3D Linear Buckling Solver'); Writeln('Version: 0.7'); Writeln('Platform: Windows x64');
...
begin
  F:=''; CaseID:=0;
  if ParamCount=0 then begin Writeln('Usage: FEM3D_Buckling.exe [--info] [--validate] [--solve] [--case N] input.fem3d'); Halt(2); end;
