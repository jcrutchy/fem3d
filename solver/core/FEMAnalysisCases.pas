...
uses Classes, SysUtils, FEMMatrix;
...
var U:string;
begin
  U:=UpperCase(Trim(S)); Result:=True;
...
function TextBool(const S:string):Boolean; begin Result:=(S='1') or SameText(S,'true') or SameText(S,'yes'); end;
...
begin
  if SameText(Key,'Solver') then SolverName:=Value
  else if SameText(Key,'MatrixStorage') and StringToMatrixStorage(Value,V) then MatrixStorage:=V
  else if SameText(Key,'PivotTolerance') then PivotTolerance:=StrToFloat(Value)
  else if SameText(Key,'ResidualTolerance') then ResidualTolerance:=StrToFloat(Value)
  else if SameText(Key,'CheckEquilibrium') then CheckEquilibrium:=TextBool(Value)
  else if SameText(Key,'CheckEnergy') then CheckEnergy:=TextBool(Value)
  else if SameText(Key,'RecoverElementForces') then RecoverElementForces:=TextBool(Value)
  else if SameText(Key,'StoreSolverData') then StoreSolverData:=TextBool(Value);
end;
...
begin
  ...
  else if SameText(Key,'EigenvalueCount') then EigenvalueCount:=StrToIntDef(Value,EigenvalueCount)
  else if SameText(Key,'Shift') then Shift:=StrToFloatDef(Value,Shift)
  else if SameText(Key,'Tolerance') then Tolerance:=StrToFloatDef(Value,Tolerance)
  else if SameText(Key,'MaxIterations') then MaxIterations:=StrToIntDef(Value,MaxIterations)
  else if SameText(Key,'NormalizeModes') then NormalizeModes:=TextBool(Value)
  else if SameText(Key,'StoreModeShapes') then StoreModeShapes:=TextBool(Value)
  else if SameText(Key,'CheckSymmetry') then CheckSymmetry:=TextBool(Value);
end;
...
begin
  if SameText(Key,'Solver') then SolverName:=Value
  else if SameText(Key,'InitialLoadSteps') then InitialLoadSteps:=StrToIntDef(Value,InitialLoadSteps)
  else if SameText(Key,'MaxIterations') then MaxIterations:=StrToIntDef(Value,MaxIterations)
  else if SameText(Key,'ForceTolerance') then ForceTolerance:=StrToFloatDef(Value,ForceTolerance)
  else if SameText(Key,'DisplacementTolerance') then DisplacementTolerance:=StrToFloatDef(Value,DisplacementTolerance)
  else if SameText(Key,'MinimumStep') then MinimumStep:=StrToFloatDef(Value,MinimumStep)
  else if SameText(Key,'MaximumStep') then MaximumStep:=StrToFloatDef(Value,MaximumStep)
  else if SameText(Key,'LineSearch') then LineSearch:=TextBool(Value)
  else if SameText(Key,'MaxCutbacks') then MaxCutbacks:=StrToIntDef(Value,MaxCutbacks)
  else if SameText(Key,'StoreIterationHistory') then StoreIterationHistory:=TextBool(Value);
end;
